---
name: thesis-polish
description: "中文理工科研究生毕业论文的写作风格约束与审校：60 多条规则、两个核心自检问题、基于范例论文对照的工作流，另有一层可本地化的论文专属约束（术语禁用表、题目口径、LaTeX 保护规则、章节地图）。双模式：①写作模式——撰写/扩写正文时即按规范下笔 ②审校模式——对已写段落/章节做诊断与改写建议。核心目标是让论文读起来像活人写的毕业论文，不露 AI 痕迹，也不滑向过度口语。触发于用户请求撰写、润色、审校中文毕业论文正文、章节、摘要、引言、结论、方法说明、实验描述、图表题注等场景；也触发于「写第 X 章」「改论文」「润色这段」「审一下这章」「有没有 AI 味」。"
user-invocable: true
...
**任何写作或审校动作之前，先读 [local-constraints.md](local-constraints.md)。**

该文件是**你这篇论文**的专属约束层，优先级高于本文件与所有 patterns。
出厂状态是一份模板，用之前先把里面的 `<...>` 占位符换成真实值——不填也能跑，
但会退化成纯通用规则模式。

填好之后它提供：

- **术语硬约束**：禁用写法表（紧耦合→紧组合、鲁棒→抗差、均方根→平方根信息滤波 等）+ 缩写大小写规范
- **题目口径约束**：选题阶段定死的边界，不得违反
- **框架口径**：全文统一的一句话表述
- **LaTeX 保护规则**：哪些结构绝对不动、禁止新增什么、裸 `%` 是硬错误
- **章节地图**：各章的功能、来源成果、审校要点
- **领域用词参考**：GNSS / 测绘惯用词对照，避免外行译法

规则冲突时按 local-constraints.md 第八节的优先级判定。

**特别地**：范例测试（范例论文里会这么写吗）遇到术语约束时，
**以 local-constraints.md 为准，不跟着范例走**。
...
```
0. 读 local-constraints.md（论文专属硬约束：术语表 / 题目口径 / LaTeX 保护 / 章节地图）
...
```
0. 读 local-constraints.md。若审的是 .tex，重点过第五节的 LaTeX 保护规则
   和第二节的术语禁用表（后者是硬命中，直接列必改）
...
对 `.tex` 文件，尊重 LaTeX 指令：`\ref`、`\cite`、`\begin{}..\end{}` 等结构不要动，只改中文正文。公式内部（`$...$`、`\begin{equation}...\end{equation}`）不改。完整清单见 local-constraints.md 第五节。
...
**`local-constraints.md` 是所有场景的第 0 必读项**，下表列的是它之外还要加载什么。

| 场景 | 必读 | 建议读 |
|------|------|--------|
| 所有场景 | **local-constraints.md** | - |
...
| 改 `.tex` 源文件 | 上面 + **local-constraints.md 第五节（LaTeX 保护）** | - |

具体模块：
- [local-constraints.md](local-constraints.md) — **论文专属硬约束（最高优先级）**
...
### 论文附加硬规则（来自 local-constraints.md，优先级同上）

以下五条的具体内容由 `local-constraints.md` 决定。该文件未填时，只剩第 23、24 条生效。

21. **术语一致性优先于正确性**：同一概念全篇只用一种写法，即使另一种也正确。
    具体禁用表见 local-constraints.md 第二节。
22. **缩写大小写全篇严格一致**，含连字符与否。清单见 local-constraints.md 第二节。
23. LaTeX：`\cite`/`\ref`/`\label` 与环境结构不动；不新增 `\zihao`/`\vspace` 等版式命令；
    正文里不出现手写的「图 3-2」「式（3-5）」（必须来自 `\ref`）
24. **裸 `%` 是硬错误**：`覆盖率95%。` 会静默吞掉整行，百分号必须写 `\%`
25. **措辞服从题目口径**：题目口径里定死的边界（哪些词进题目、哪些场景怎么说）
    不得在正文里偏离。见 local-constraints.md 第三节。
